            ...
            <form id="multi-step-form">
                ...
                <div class="step step-5">
                    <div class="step-78 text-center">
                        ...
                        <ul class="nav nav-tabs intro-tabs tabs-box" id="myTab" role="tablist">
                            ...
                            <li class="nav-item tab-btns">
                                <a class="nav-link tab-btn eng_proficiency_level" data-eng="{{$item->id}}" href="javascript:void(0)"
                                    ...
                                    onclick="activateTab('{{$item->name}}', this)">
                                    {{$item->name}}
                                </a>
                            </li>
                            @endforeach
                        ...
                        @foreach ($eng_proficiency_level as $item)
                        <div class="tab-pane tab input-data mt-5" id="{{$item->name}}" role="tabpanel" aria-labelledby="{{$item->name}}-name"
                            style="display:none;">
                            <div class="content pt-30">
                                <div class="course-overview">
                                    <div class="inner-box">
                                        <input class="form-control" type="text" id="{{$item->name}}-input" name="{{$item->name}}-input"
                                            placeholder="Enter Your Score" oninput="validateScore('{{$item->name}}')">
                                        <br>
...
<script>
    ...
    function setupTabClickHandler() {
        $('.program-sub-level-data,.data_education_level').off('click').on('click', function() {
            $('.program-sub-level-data,.data_education_level').removeClass('active').attr('aria-selected', 'false');
...
<script>
    ...
    $(document).on('click', '.other-exam-data', function () {
        $('.other-exam-data').removeClass('active');
        $(this).addClass('active');
    });
